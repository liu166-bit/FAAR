...
class visible_module(nn.Module):
    def __init__(self, arch='resnet50'):
        ...
        self.GlobalFilter = GlobalFilter(64)
    def forward(self, x):
        
        x = self.GlobalFilter(x)
...
class GlobalFilter(nn.Module):
    def __init__(self, dim, h=14, w=8,mask_alpha=0.5,):
        super().__init__()
        self.complex_weight = nn.Parameter(torch.randn(1,dim,2, dtype=torch.float32) * 0.02)
        self.complex_weight
        self.w = w
        self.h = h
    ...
    def forward(self, x, spatial_size=None):
        ...
        x_fft_amplitude = torch.abs(x_fft)
        x_fft_phase = torch.angle(x_fft)
        #if(c == 64):
        #x_fft_amplitude = self.sa(x_fft_amplitude) * x_fft_amplitude
        x_fft_phase = self.sa(x_fft_phase) * x_fft_phase
     
        
        
        # 重新构建频域表示
        x_fft_reconstructed = x_fft_amplitude * torch.exp(1j * x_fft_phase)
        
        # 进行逆频域变换
        x_reconstructed = torch.fft.irfft(x_fft_reconstructed, norm='ortho')
