...
class AQALoss(nn.Module):
    def __init__(self, margin=0.2):
        super(AQALoss, self).__init__()
        self.margin = margin
    ...
    def forward(self, inputs, targets):
        ...
        for i in range(n):
            dist_ap_123.append(dist_23[i][mask[i]].min().unsqueeze(0))
            dist_an_123.append(dist_13[i][mask[i]].max().unsqueeze(0))
            dist_an_33.append(dist_33[i][mask[i] == 0].min().unsqueeze(0))
            dist_an_11.append(dist_11[i][mask[i] == 0].min().unsqueeze(0))

            dist_ap_124.append(dist_14[i][mask[i]].min().unsqueeze(0))
            dist_an_124.append(dist_24[i][mask[i]].max().unsqueeze(0))
